...
class LegacyGoogleDriveProvider extends ProviderInterface {
    ...
    if (authCallback) {
      if (this.authToken) {
        ...
        if (options?.forceAuthorization) {
          ...
          return this.doAuthorize(LegacyGoogleDriveProvider.IMMEDIATE)
        }
      }
    } else {
  ...
  authorize(callback:any) {
    ...
    // dialog, if necessary
    this.doAuthorize(!LegacyGoogleDriveProvider.IMMEDIATE)
  }
  ...
  waitForAPILoad() {
    return LegacyGoogleDriveProvider.apiLoadPromise || (LegacyGoogleDriveProvider.apiLoadPromise = Promise.all([this.waitForGISLoad(), this.waitForGAPILoad()]))
  }

  private waitForGAPILoad() {
    return LegacyGoogleDriveProvider.gapiLoadPromise || (LegacyGoogleDriveProvider.gapiLoadPromise = new Promise<void>((resolve, reject) => {
  ...
  private waitForGISLoad() {
    return LegacyGoogleDriveProvider.gisLoadPromise || (LegacyGoogleDriveProvider.gisLoadPromise = new Promise<void>((resolve) => {
      const script = document.createElement('script')
...
export default LegacyGoogleDriveProvider
